...
def test_dataclass_roundtrip(store: Store) -> None:
    ...
    u = DUser(id=1, name="Anna", age=30, role=Role.admin, created=datetime(2021, 1, 2, 3, 4), tags=["a", "b"], nickname="ann")
    users.put(u)
...
def test_pydantic_roundtrip(store: Store) -> None:
    ...
    u = PUser(id=1, name="Bob", age=25, role=Role.user, created=datetime(2021, 1, 2, 3, 4), tags=["q"])
    users.put(u)
    assert users.get(1) == u
...
def test_msgspec_roundtrip(store: Store) -> None:
    ...
    u = MUser(id=1, name="Vera", age=40, role=Role.admin, created=datetime(2021, 1, 2, 3, 4), tags=["z"])
    users.put(u)
    assert users.get(1) == u
